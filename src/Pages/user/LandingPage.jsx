...
function LandingPage() {
  ...
  const [filteredJobs, setFilteredJobs] = useState([]) // searched jobs
              ...
              <div className='px-2 mih-h-fit py-1 gap-2 flex justify-between overflow-hidden rounded items-center bg-white'>
                <div className='px-2 flex items-center gap-2'>
                  ...
                  <input
                    ...
                    className='outline-none text-[13px] md:text-[15px] w-25 md:w-29'
        ...
        <div className='md:flex mt-5 pb-10'>
          ...
          <div className={filterOpen ? "" : 'max-lg:hidden'}>
            {/* <div className='px-5 md:px-15 space-y-1.5 mt-5 md:mt-10'>
              ...
              {["Programming", "Data Science", "Designing", "Networking", "Management", "Marketing", "Cyber Security","React developer"].map((cat) => (
                <div key={cat} className='gap-1 flex'>
                  <input type="radio" name="category" id={cat} value={cat} checked={selectedCategory === cat} onChange={(e) => setSelectedCategory(e.target.value)} />
            ...
            <div className='px-5 md:px-15 space-y-1.5 mt-5'>
              ...
              {["Banglore", "Washington", "Hyderabad", "Mumbai", "California", "Chennai", "New York","TRIVANDRUM"].map((loc) => (
                <div key={loc} className='gap-1 flex'>
                  <input type="radio" name="location" id={loc} value={loc} checked={selectedLocation === loc} onChange={(e) => setSelectedLocation(e.target.value)} />
          ...
          {/* Job Cards */}
          <div className='md:mt-5 px-5 md:px-15 mt-10 w-full'>
            ...
            <div className='flex gap-2 mt-5 justify-center'>
              {[1, 2, 3].map((num) => (
                <button
                  key={num}
                  className='w-10 h-10 rounded border border-blue-600 bg-blue-50 text-blue-800 hover:bg-blue-400 hover:text-gray-500'
                >
                  {num}
                </button>
              ))}
